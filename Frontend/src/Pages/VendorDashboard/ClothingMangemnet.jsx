...
const ClothingManagement = () => {
  ...
  const { getInventory, products: dashboardData, loading } = useProductStore();

  useEffect(() => {
    getInventory("Clothing");
  }, [getInventory]);
  ...
  const handleSaveEdit = () => {
    ...
    setClothingItems((items) =>
      items.map((item) => (item._id === editingItem ? { ...editForm } : item))
    );
    setEditingItem(null);
    setEditForm({});
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-indigo-600">
            Manage your clothing inventory and product catalog
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
          {clothingItems.map((item) => (
            <div
              key={item._id}
              className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-indigo-100 transition-all duration-300 hover:shadow-3xl"
            >
              {editingItem === item._id ? (
                ...
                <>
                  ...
                  <div className="relative h-64 bg-gradient-to-br from-indigo-100 to-indigo-200">
                    ...
                    {/* Gender Badge */}
                    <div
                      className={`absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-medium ${getGenderColor(
                        item.gender
                      )}`}
                    >
                      {item.gender}
        ...
        <div className="mt-12 text-center text-indigo-600 text-sm">
          <p>
            Total Items: {clothingItems.length} | Need help? Contact{" "}
