...
const MedicalStaffingDashboard = () => {
  ...
  const addIncludedThing = (scheduleIndex) => {
    setFormData((prev) => ({
      ...
      schedule: prev.schedule.map((item, index) =>
        index === scheduleIndex
          ? { ...item, includedItems: [...item.includedItems, ""] }
          : item
      ),
    }));
  };
    ...
    setFormData((prev) => ({
      ...
      schedule: prev.schedule.map((item, index) =>
        index === scheduleIndex
          ? {
              ...item,
              includedItems: item.includedItems.filter(
                (_, i) => i !== thingIndex
              ),
            }
          : item
    ...
    setFormData((prev) => ({
      ...
      schedule: prev.schedule.map((item, index) =>
        index === scheduleIndex
          ? {
              ...item,
              includedItems: item.includedItems.map((thing, i) =>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                {formData.schedule.map((schedule, scheduleIndex) => (
                  <div
                    key={scheduleIndex}
                    className="bg-indigo-50 rounded-lg p-4 mb-4 border border-indigo-100"
                  >
                      ...
                      <div>
                        ...
                        {schedule.includedItems.map((thing, thingIndex) => (
                          ...
                          >
                            ...
                            />
                            {schedule.includedItems.length > 1 && (
                              <button
                                onClick={() =>
                                  removeIncludedThing(scheduleIndex, thingIndex)
                                }
      ...
      <div className="bg-white shadow-lg">
        <div className="px-4 sm:px-6 py-6">
          <div className="text-center">
            ...
            {/* Add Staff Button */}
            <Link to={"/dashboard/vendor/add/medical/doctors"}>
              <div className="flex justify-center">
                <button className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm sm:text-base font-medium py-2 px-4 rounded-lg shadow-md transition-all duration-200">
                  + Add Staff
                </button>
              </div>
            </Link>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {[...new Set(staffingData.map((item) => item.type))].length}
                </p>
              </div>
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="space-y-4 sm:space-y-6">
          {filteredData.map((item) => (
            <div
              key={item._id}
              className="bg-white rounded-2xl shadow-xl overflow-hidden border border-indigo-100 transition-all hover:shadow-2xl"
            >
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-start flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-indigo-200 mt-1 text-sm sm:text-base line-clamp-2">
                        {item.description}
                      </p>
                    </div>
                  </div>
              ...
              <div className="p-4 sm:p-6">
                ...
                <div className="space-y-4">
                  {item.schedule.map((schedule, scheduleIndex) => (
                    <div
                      key={scheduleIndex}
                      className="bg-indigo-50 rounded-xl p-4 sm:p-5 border border-indigo-100"
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 space-y-2 sm:space-y-0">
                        <h5 className="font-semibold text-indigo-800 text-sm sm:text-base">
                          {schedule.name}
                        </h5>
                      ...
                      <div>
                        <p className="text-xs sm:text-sm font-medium text-indigo-800 mb-2">
                          Included Services:
                        </p>
