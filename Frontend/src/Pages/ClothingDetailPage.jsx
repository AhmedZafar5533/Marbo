                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                            ...
                            <div className="flex-grow">
                                ...
                                {/* Product Title */}
                                <h1 className="text-2xl md:text-3xl font-bold">{productDetails.productName}</h1>

                                {/* Brand */}
                                <div className="flex items-center mt-2">
                                    <span className="text-neutral-600 text-sm">By </span>
                                    <span className="text-neutral-800 font-medium text-sm ml-1">{productDetails.brand}</span>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-3 mt-4">
                                    ...
                                    <div className="flex items-center bg-neutral-50 px-4 py-2 rounded-lg">
                                        ...
                                        <span className="text-sm font-medium">{productDetails.gender}</span>
                                    </div>
                                    ...
                                    {productDetails.quantity > 0 && (
                                        <div className="flex items-center bg-neutral-50 px-4 py-2 rounded-lg">
                                            ...
                                            <span className="text-sm font-medium">{productDetails.quantity} available</span>
                                        </div>
                                    )}
                                </div>
                                ...
                                {productDetails.sizes && productDetails.sizes.length > 0 && (
                                    <div className="mt-4">
                                        <h3 className="text-sm font-medium text-neutral-700 mb-2">Size</h3>
                                        <div className="flex flex-wrap gap-2">
                                            {productDetails.sizes.map((size, idx) => (
                                                <button
                                                    key={idx}
                                                    className={`px-3 py-2 text-sm border rounded-lg transition-colors ${selectedSize === size
                            ...
                            <div className="flex flex-col items-start md:items-end space-y-3">
                                <div className="flex items-baseline">
                                    <p className="text-3xl font-bold text-neutral-900">{formatPrice(productDetails.price)}</p>
                                </div>
                                <button
                                    onClick={() => buyHandler(productDetails._id, productDetails.price)}
                                    disabled={productDetails.quantity === 0}
                                    className={`font-medium py-3 px-8 rounded-lg shadow-sm hover:shadow transition-all flex items-center gap-2 ${productDetails.quantity > 0
                                        ? 'bg-red-600 hover:bg-red-700 text-white'
                                        : 'bg-neutral-300 text-neutral-500 cursor-not-allowed'
                                        }`}
                                >
                                    <MessageSquare size={18} />
                                    {productDetails.quantity > 0 ? 'Place Order' : 'Out of Stock'}
                                </button>
                            </div>
                        </div>
                                ...
                                {activeTab === "description" && (
                                    <div className="animate-fadeIn">
                                        ...
                                        <p className="text-neutral-700 leading-relaxed mb-6">
                                            {productDetails.description || "High-quality product with attention to detail and customer satisfaction."}
                                        </p>
                                        ...
                                        <div className="space-y-4">
                                            <div className="grid grid-cols-2 gap-4 p-4 bg-neutral-50 rounded-lg">
                                                <div>
                                                    ...
                                                    <p className="text-neutral-900">{productDetails.brand}</p>
                                                </div>
                                                <div>
                                                    <span className="text-sm font-medium text-neutral-600">Category</span>
                                                    <p className="text-neutral-900">{productDetails.category}</p>
                                                </div>
                                                <div>
                                                    <span className="text-sm font-medium text-neutral-600">Gender</span>
                                                    <p className="text-neutral-900">{productDetails.gender}</p>
                                                </div>
                                                <div>
                                                    <span className="text-sm font-medium text-neutral-600">Stock</span>
                                                    <p className="text-neutral-900">{productDetails.quantity} units</p>
                                                </div>
                                            </div>

                                            {productDetails.sizes && productDetails.sizes.length > 0 && (
                                                <div className="p-4 bg-neutral-50 rounded-lg">
                                                    <span className="text-sm font-medium text-neutral-600 block mb-2">Available Sizes</span>
                                                    <div className="flex flex-wrap gap-2">
                                                        {productDetails.sizes.map((size, idx) => (
                                                            <span key={idx} className="px-2 py-1 bg-white border border-neutral-200 rounded text-sm">
                                                                {size}
                                                            </span>
                                                        ))}
                                                    </div>
                                ...
                                <div className="p-5">
                                    ...
                                    <div className="mb-4">
                                        <div className="flex items-center justify-between text-sm mb-2">
                                            ...
                                            <span className={`font-medium ${productDetails.quantity > 0
                                                ? 'text-green-600'
                                                : 'text-red-600'
                                                }`}>
                                                {productDetails.quantity > 0 ? 'In Stock' : 'Out of Stock'}
