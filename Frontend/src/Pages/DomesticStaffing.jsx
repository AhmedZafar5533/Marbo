...
const DomesticStaffingService = () => {
    ...
    const { getAllDomesticStaffing, domesticStaffing, loading, serviceData } = useDomesticStaffingStore();
    ...
    const getServiceColor = () => { // No 'type' parameter needed anymore
    ...
    if (!loading && domesticStaffing.length === 0) return (
        <div className="min-h-screen bg-gradient-to-br from-red-50 via-orange-50 to-red-100 flex items-center justify-center p-4">
            <div className="bg-white shadow-xl rounded-2xl p-8 max-w-md text-center animate-fade-in">
                ...
                <p className="text-slate-500">We couldn't find any domestic staffing services at the moment. Please check back later.</p>
            </div>
        </div>
    );
            ...
            <div className="relative max-w-7xl mx-auto px-4 py-8">
                ...
                <section className="text-center mb-16 relative">
                    <div className="animate-fade-in-up">
                        <div className="inline-flex items-center gap-3 bg-white/80 backdrop-blur-sm px-6 py-3 rounded-full border border-red-200 mb-6 shadow-lg">
                            ...
                            <span className="text-sm font-medium text-red-700">Trusted & Verified</span>
                        </div>
                    ...
                    <div className="bg-white/90 backdrop-blur-xl shadow-2xl rounded-3xl p-8 border border-white/50 hover:shadow-3xl transition-all duration-500 hover:-translate-y-2">
                        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h2 className="text-3xl font-bold text-slate-800 mb-2">{serviceData.vendorName}</h2>
                ...
                <section className="mb-16">
                    <div className="text-center mb-12">
                        <h3 className="text-4xl font-bold text-slate-800 mb-4">Step 1: Choose Your Service</h3>
                        <p className="text-xl text-slate-600">Select the type of professional staff you need</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {domesticStaffing.map((service, index) => (
                            <div
                                key={service._id}
                                className={`group cursor-pointer transition-all duration-500 hover:-translate-y-3 ${selectedService === service._id
                            ...
                            >
                                <div className="rounded-2xl p-6 h-full">
                                    <div className={`mb-4 group-hover:scale-110 transition-transform duration-300 ${selectedService === service._id ? 'scale-110' : ''}`}>
                                        <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${selectedService === service._id ? 'from-white/20 to-white/10' : getServiceColor(service.type)} flex items-center justify-center shadow-lg`}>
                                            <span className={`text-2xl font-bold ${selectedService === service._id ? 'text-white' : 'text-white'}`}>
                                                {index + 1}
                                            </span>
                                        </div>
                                    </div>
                                    <h4 className={`text-xl font-bold mb-2 ${selectedService === service._id ? 'text-white' : 'text-slate-800'}`}>
                                        {service.type}
                                    </h4>
                                    <p className={`mb-4 ${selectedService === service._id ? 'text-red-100' : 'text-slate-600'}`}>
                                        {service.description}
                                    </p>
                                    <div className={`text-sm font-semibold ${selectedService === service._id ? 'text-white' : 'text-red-600'}`}>
                                        Starting from ${Math.min(...service.schedule.map(s => s.rate))}/week
                                    </div>
                ...
                {selectedService && (
                    <section className="mb-16">
                        <div className="text-center mb-12">
                            ...
                            <p className="text-xl text-slate-600">Choose how many days per week you need service</p>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {domesticStaffing.find(s => s._id === selectedService)?.schedule.map((schedule) => (
                                <div
                                    key={schedule.name}
                                ...
                                >
                                    <div className="rounded-3xl p-8">
                                        ...
                                        <div className="text-center">
                                            <p className={`text-sm ${selectedSchedule === schedule.name ? 'text-red-100' : 'text-slate-600'}`}>
                        ...
                        <div className="bg-gradient-to-r from-rose-500 to-red-500 rounded-3xl p-12 text-white text-center">
                            ...
                            <div className="flex flex-col md:flex-row items-center justify-center gap-8">
                                ...
                                <div className="text-left">
                                    <div className="mb-2">
                                        <span className="font-semibold">Service:</span> {domesticStaffing.find(s => s._id === selectedService)?.type}
                                    </div>
                                    <div className="mb-2">
                                        <span className="font-semibold">Schedule:</span> {selectedSchedule}
                                    </div>
                                    <div className="mb-4">
                                        <span className="font-semibold">Days:</span> {domesticStaffing.find(s => s._id === selectedService)?.schedule.find(sch => sch.name === selectedSchedule)?.days} days/week
                                    </div>
                ...
                <section className="mb-16">
                    <div className="text-center mb-12">
                        <h3 className="text-4xl font-bold text-slate-800 mb-4">See Our Work in Action</h3>
                        <p className="text-xl text-slate-600">Professional excellence captured in every moment</p>
                    </div>
